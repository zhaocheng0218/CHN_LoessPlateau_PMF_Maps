////**********************************
////**Codes for exporting final maps.

var city = ee.FeatureCollection("users/my-work/LoessPlateau_PFM/LP_city_nominal")
            .filter(ee.Filter.eq("City","Henan")).geometry();
var roi = city
Map.addLayer(roi,{color:"red"},"roi",false);

var pfm1 = ee.FeatureCollection("users/my-work/LoessPlateau_PFM/samples_0812/pfm_HANTS_buffered");
var pfm2 = ee.FeatureCollection("users/my-work/LoessPlateau_PFM/samples_0812/pfm_HANTS_SUPPLE_buffered");
var pfm = pfm1.merge(pfm2)
            .filterBounds(roi)
            ...
            .select(["peak_green","peak_bsi","peak_dbsi","peak_nmdi","peak_pmli","peak_swir2","peak_gcvi",
                       ...
                       "swir2_constant","gcvi_constant","landcover"])
            //.limit(1500);
var nopfm = ee.FeatureCollection("users/my-work/LoessPlateau_PFM/samples_0812/nopfm_HANTS_buffered")
              .filterBounds(roi)
...
// print("pfm:",pfm.first());
// print("pfm:",pfm.size());
// print("nopfm",nopfm.size());
var training = pfm.merge(nopfm);

var bandNames = training.first().propertyNames().removeAll(["random","system:index","landcover"]);
...
var s2Col = s2Col.map(maskS2clouds)
    //oeel.Algorithms.Sentinel2.cloudfree(80, s2Col)
...
var rgb = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
              .filterDate("2019-05-10","2019-05-15")
              .filterBounds(roi)
              //.filter(ee.Filter.lt("CLOUDY_PIXEL_PERCENTAGE",70))
              .map(maskS2clouds)
              .select(oldBands,newBands)
              .max().clip(roi);
Map.addLayer(rgb,{bands:["red","green","blue"],min:500,max:3200},"rgb",false);
// Map.addLayer(image,{min:0,max:1},"image",false);

Map.addLayer(pfm,{color:"yellow"},"pfm",false);
Map.addLayer(nopfm,{color:"#20d677"},"nopfm",false);

////+++ Add time band and constant band.
function addVaribles(img){
  var date_img = ee.Date(img.get("system:time_start"));
  var years_img = date_img.difference(ee.Date("2019-01-01"), "year");
...
var data = green_coeffi.addBands(swir2_coeffi)
                      .addBands(gcvi_coeffi).addBands(nmdi_coeffi)
                      .addBands(pmli_coeffi).addBands(bsi_coeffi).addBands(dbsi_coeffi)
                      .addBands(green_peak).addBands(swir2_peak)
                      .addBands(gcvi_peak).addBands(nmdi_peak).addBands(pmli_peak).addBands(bsi_peak)
                      .addBands(dbsi_peak);
...
var pfmResult = classified.updateMask(classified);
Map.addLayer(pfmResult,{palette:["yellow"]},"pfmResult",false);
...
Export.image.toAsset({
  ...
  crs:"EPSG:4326",
  maxPixels:1e13,
  description:"LoessPlateau_PFM/PFM_results_2019/pfm_Henan2019"
});
